...
public:
    ...
    static  vector<string> Split(string S1, string delim) {
        ...
        if (S1 != "")
        {
            words.push_back(S1); // it counts the last word of the string.
        }
    ...
    short CountWords()
    {
     ...
     static char UpperLetter(char letter) {
         char newLetter =  0 ;
        !IsUpper(letter) ?
           newLetter = letter - 32 : newLetter = letter;
        return newLetter;
    }
   ...
   static vector<char> splitWord(string word) {
       vector<char> letter;
       for (int i = 0; i < word.length(); i++) {
   ...
   static string JoinLetter(vector<char> letters) {
       string newWord = "";
   ...
   static bool IsLetter(char letter) {
       return   (letter >= 65 && letter <= 90   || letter >= 97 && letter <= 122) ? true : false;
   }
